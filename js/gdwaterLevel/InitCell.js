...
class InitCell {
    ...
    _computerWaterFlow(i,j,k1,k2,t1,t2){
        ...
        return {'cellSlop':cellSlop,'waterFlow':waterFlow}
    }
            ...
            if (element.waterFlow) {
                ...
                if (results.length != 0) {
                    results.forEach((element2) => {
                        ...
                        nextPollutedCell.cellMass += (1/Math.sqrt(2))* this._waterVeloty * distance * (element.cellMass - nextPollutedCell.cellMass) 
                        nextPollutedCell.isPolluted = true
